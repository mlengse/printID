...
2.  Di halaman Options:
    *   **URL Sistem**: Masukkan alamat URL dasar dari sistem manajemen Anda (Contoh: `http://192.168.100.178/app/`).
...
*   `background.js`: Service worker yang menangani event navigasi dan injection script.
*   `content_script.js`: Script utama untuk scraping data dan generate PDF label identitas.
*   `resep.js`: Script untuk scraping data dan generate PDF label obat.
*   `health_screening.js`: Script untuk deteksi dan verifikasi skrining.
*   `captcha_solver.js`: Logic untuk menyelesaikan CAPTCHA menggunakan ONNX Runtime.
