if ($('#label2PDF').length === 0) {
  ...
  function pasienUm() {
    const tanggalSplit = $('#tgllahir').val().split('-');
    const dateBirth = new Date(tanggalSplit[2] + "-" + tanggalSplit[1] + "-" + tanggalSplit[0]);
  ...
  $("#label2PDF").bind('click', function () {
    ...
    while (pasienNama && pasienNama.length > 30) {
    ...
    if (pasienUmur && !pasienUmur.length) {
      alert('mohon cek tgl lahir dan umur')
    } else {
      if (pasienUmur) {
        const pasienThn = Number(pasienUmur.split('thn')[0].trim())
        const pasienBln = Number(pasienUmur.split('thn')[1].split('bln')[0].trim())
        const pasienHr = Number(pasienUmur.split('thn')[1].split('bln')[1].split('hr')[0].trim())
...
if ($('#drug2PDF').length === 0) {
  ...
  if ($(`#${el} > table.nested-table > tbody > tr`).length > 1) {
    ...
    obats = [...document.getElementById(el).querySelectorAll('tr')].map(row => {
      const tabs = [...row.querySelectorAll('td')]
      if (!tabs) {
        return {}
      }
      if (!tabs[1]) {
    ...
    for (const [id, obat] of Object.entries(obats)) if (Number(id) > 0 && obat.kode) {
      const labelID = `${id}.${obat.kode}`
      const child = Number(id) + 1
  ...
  function clickLabel(a) {
    const obat = Object.assign({}, obats[a.currentTarget.id.split('.')[0]])
    obat.ket = ''
    obat.ket2 = ''
    ...
    if (!dataPasien.umur.length) {
      ...
      // console.log(dataPasien)
      const pasienThn = Number(dataPasien.umur.split('thn')[0].trim())
      const pasienBln = Number(dataPasien.umur.split('thn')[1].split('bln')[0].trim())
      const pasienHr = Number(dataPasien.umur.split('thn')[1].split('bln')[1].split('hr')[0].trim())
    ...
    const svg = drawBarcode("svg", dataPasien.rm, {
      type: 'Code 128'
    });
